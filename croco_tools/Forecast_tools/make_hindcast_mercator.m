...
now = datenum(datestr(datenum(datetime('now', 'TimeZone','Z')),'yyyy-mm-dd'))-MERCATOR_delay;
...
%---------------------------------------------------------------
if makeini==1
  ininame=[ini_prefix,datestr(now-MERCATOR_offset,'yyyymmdd'),nc_suffix];
  disp(['Create an initial file for ',datestr(now-MERCATOR_offset,'yyyymmdd');])
  create_inifile(ininame,grdname,CROCO_title,...
                 theta_s,theta_b,hc,N,...
                 now-MERCATOR_offset-datenum(Yorig,1,1),...
                 'clobber',vtransform); % starts at 00:00
  nc_ini=netcdf(ininame,'write');
  mercator_name=[SCRATCH_dir,'/mercator_',datestr(now-MERCATOR_offset,'yyyymmdd'),'.cdf'];
  interp_OGCM_frcst(mercator_name,Roa,interp_method,...
