     ...
     &                                ,   vname
      ...
      real POT_Tamp(-1:Lm+2+padd_X,-1:Mm+2+padd_E,Ntides)
      common /tides_POT_Tamp/ POT_Tamp
      real POT_Tphase(-1:Lm+2+padd_X,-1:Mm+2+padd_E,Ntides)
      common /tides_POT_Tphase/ POT_Tphase
      real PTide(-1:Lm+2+padd_X,-1:Mm+2+padd_E)
     ...
     &        status, varid,ind
      ...
      ind=itide
        ...
        status=nf_fread(SSH_Tamp(-1,-1,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        ...
        status=nf_fread(SSH_Tphase(-1,-1,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        if (status.ne.NF_NOERR) then
        ...
        status=nf_fread(UV_Tangle(-1,-1,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        if (status.ne.NF_NOERR) then
        ...
        status=nf_fread(UV_Tphase(-1,-1,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        if (status.ne.NF_NOERR) then
        ...
        status=nf_fread(UV_Tmajor(-1,-1,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        if (status.ne.NF_NOERR) then
        ...
        status=nf_fread(UV_Tminor(-1,-1,itide),
     &                              ncidfrc,varid,ind,r2dvar)
        if (status.ne.NF_NOERR) then
        ...
        status=nf_fread(POT_Tamp(-1,-1,itide),
     &                              ncidfrc,varid,itide,r2dvar)
        ...
        status=nf_fread(POT_Tphase(-1,-1,itide),
     &                              ncidfrc,varid,itide,r2dvar)
        if (status.ne.NF_NOERR) then
